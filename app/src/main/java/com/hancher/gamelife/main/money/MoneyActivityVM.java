 ...
 * 描述 : 点击打卡界面
 */
public class MoneyActivityVM extends VMBaseActivity<ColockInActivityBinding, MoneyViewModel> {
    private MoneyAdapter adapter = new MoneyAdapter();
    ...
    private void initObseve() {
        ...
        adapter.setOnItemClickListener((adapter, view, position) -> {
            ...
            inputDialog = new MaterialDialog.Builder(MoneyActivityVM.this)
                    .title("备注")
                    .input("在这里填写备注", "", (dialog, input) -> {
                        viewModel.addColockIn(type, input);
                        dialog.dismiss();
                        waitingDialog = new MaterialDialog.Builder(MoneyActivityVM.this)
                                .title("保存中")
                                .progress(true,100)
                                .show();
                    }).show();
        ...
        binding.btnFloat.setOnClickListener(v -> {
            Intent intent = new Intent(MoneyActivityVM.this, MoneyTypeAddActivityVM.class);
            MoneyActivityVM.this.startActivity(intent);
        });
        ...
        viewModel.getState().observe(this,state -> {
            if (state == MoneyViewModel.STATE.SAVE_FINISH){
                ...
                startActivity(new Intent(MoneyActivityVM.this, MoneyListActivityVM.class));
            }
        });
        // 长按删除功能
        adapter.setOnItemLongClickListener((adapter, view, position) -> {
            ColockInType type = (ColockInType) adapter.getItem(position);
            deleteDialog = new MaterialDialog.Builder(MoneyActivityVM.this)
                    .title("是否删除此类型？")
                    .positiveText("删除")
        ...
        // 删除成功后，更新列表
        viewModel.getState().observe(this, state -> {
            if (state == MoneyViewModel.STATE.DELETE_FINISH) {
                viewModel.queryAllType();
            }
        });
    }
    ...
    public boolean onOptionsItemSelected(@NonNull MenuItem item) {
        switch (item.getItemId()){
            case R.id.menu_item_list:
                startActivity(new Intent(this, MoneyListActivityVM.class));
                break;
